...
@app.route('/pelajar', methods=['GET','POST','PUT','DELETE'])
def mahasiswa():
    try: 
        ...
        if request.method == 'GET':
            ...
            if id_:
                query = "SELECT * FROM data_pelajar where id = %s "
                values = (id_,)
            else:
        ...
        # Jika Method POST
            ...
            usia = datainput['usia']
            query = "INSERT INTO data_pelajar (nama, pekerjaan, usia) values (%s,%s,%s) "
        ...
        #Jika Method PUT
            ...
            query = "UPDATE data_pelajar SET id = %s "
            datainput = request.json
            id_ = datainput['id']
            values += (id_,)
            ...
            # Selain itu adalah DELETE, bila ada method selain keempat ini maka dipastikan akan langsung error karena method tidak di assign.
                ...
                query = "DELETE FROM data_pelajar id = %s "
                id_ = request.args.get("id")
                values = (id_,)
                dt.insert_data(query, values)
                data = [{
